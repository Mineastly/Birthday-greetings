...
function anim(){
  ...
  // removed autoplay for meow — user gesture is required to start audio
  // if( !meowAudioStarted ){
...
// add explicit user control for meow audio (no autoplay)
var meowBtn = document.getElementById('music-btn');
if (meowBtn) {
  meowBtn.addEventListener('click', function () {
    if (!meowAudioStarted) {
      meowAudio.play().then(function () {
        meowAudioStarted = true;
      ...
      meowAudio.pause();
      meowAudioStarted = false;
      meowBtn.textContent = 'Play Music';
      meowBtn.setAttribute('aria-pressed', 'false');
    }
  });
}
